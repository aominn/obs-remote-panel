...
### ATEM Quick Actionsの見方

- **PROGRAM（赤）**は現在配信・収録に出ている入力です。「本番へ直接切り替え」で現在のATEM入力一覧から対象を選ぶと、CUT／AUTOを押さなくてもPROGRAMが即時に切り替わります。
- **PREVIEW（緑）**は次に出す入力です。「次に出す入力を選択」で現在のATEM入力一覧から対象を選びます。選択しただけではPROGRAMは変わりません。
- **CUT**を押すと、現在のPREVIEW入力がトランジションなしでPROGRAMへ即時に切り替わります。
- **AUTO**を押すと、現在のPREVIEW入力がATEM本体に設定されているトランジションでPROGRAMへ切り替わります。

ATEM未接続時、操作中、またはトランジション中はQuick Actionsを実行できません。危険操作の確認が有効な場合、PROGRAM・CUT・AUTOは実行前に確認を表示します。PREVIEWの選択では確認を表示しません。

基本操作は、PREVIEWで次の入力を確認してからCUTまたはAUTOを押します。確認を挟まず直ちに切り替える場合だけPROGRAMの入力を直接選択してください。切り替え後のPROGRAM／PREVIEW表示はATEM本体から取得した状態に更新されます。
